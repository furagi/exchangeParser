...
(function() {
  ...
  OpenExchange = (function(_super) {
    ...
    OpenExchange.prototype._parse = function(data) {
      ...
      if (data == null) {
        this._onParseError('Can\'t get from openexchange.org');
        return;
      }
      try {
      ...
      if (data.error) {
        this._onParseError(data.description);
        return;
      }
      b = data.base;
      if (b !== this._base) {
      ...
      for (i = _i = 0, _ref = this._currencys.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        cur[this._currencys[i]] = Math.round(data.rates[this._currencys[i]] / cost * 10000) / 10000;
      }
